...
3. Environment Variables (Settings → Environment Variables):
   ...
   - `ANTHROPIC_API_KEY` — ключ от console.anthropic.com (опционально).
     Если задан, на детальном экране любого айтема появляется кнопка
     `🤖 Объясни простыми словами` — бот делает короткий LLM-вызов
     (Haiku) и пришлёт 3-5 предложений на русском. Если ключ не
     задан — кнопка не показывается и фича просто отсутствует.
   - `BOT_LLM_MODEL` — id модели для `🤖 Объясни` (опционально, по
     умолчанию `claude-haiku-4-5-20251001`).
