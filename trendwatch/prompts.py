...
- not_a_skill: кандидат не является Claude Skill (это просто AI-проект,
  ...
  **ВАЖНОЕ ИСКЛЮЧЕНИЕ ИЗ ИСКЛЮЧЕНИЯ:** если репо имеет `verified=True`
  (значит `.claude/skills/<name>/SKILL.md` реально лежит в проекте) И
  stars ≥ 1000 — НЕ помечай как not_a_skill, а помещай в `top_watch` с
  пояснением вида "N skills внутри X-проекта (репо сам по себе — это X,
  но skills могут быть полезны отдельно)". Пользователь хочет знать о
  любых доступных skills, даже если они часть большого проекта.
