...
- веса моделей, датасеты, бенчмарки, туториалы, шаблоны-заготовки
  без запускаемого продукта, демки без деплоя, заброшенные репо.
- **awesome-СПИСКИ и любые коллекции ссылок** (репо вида `awesome`, `awesome-*`,
  curated lists) — это НЕ софт, их нельзя развернуть/ребрендить. НИКОГДА не
  продвигай их в top_test/top_watch, даже если у них сотни тысяч звёзд. Большое
  число звёзд НЕ делает список продуктом — клади в excluded.not_a_product.

==== ВЫСОКОЗВЁЗДНЫЕ ЗРЕЛЫЕ ПРОДУКТЫ ====
Если репо — явно ГОТОВЫЙ деплоибельный продукт/платформа (есть Docker/CLI/
installer/hosted, UI и/или API) И у него много звёзд (≥1000), это ВАЛИДНЫЙ
кандидат в top_test, даже если он «всем известен» и не нов (например n8n,
immich, open-webui). НЕ занижай такие из-за низкой novelty — для готовых
продуктов важны utility/testability/business_impact (ребренд/API/свой сервис),
а не новизна. Список-или-не-продукт — отдельно (см. выше), но настоящий
продукт с большими звёздами продвигай.
